...
BoostHttpOnlySslClient::BoostHttpOnlySslClient() : _httpParser() {
    ...
    _socket = std::make_unique<boost::asio::ssl::stream<boost::asio::ip::tcp::socket>>(_ioService, _context);
}
...
string BoostHttpOnlySslClient::makeRequest(const Url& url, const vector<HttpReqArg>& args) const {
    ...
    connect(_socket->lowest_layer(), resolver.resolve(query));
    ...
    _socket->set_verify_callback(ssl::rfc2818_verification(url.host));

    _socket->handshake(ssl::stream<tcp::socket>::client);

    string requestText = _httpParser.generateRequest(url, args, false);
    write(*_socket, buffer(requestText.c_str(), requestText.length()));
    ...
    int nativeSocket = static_cast<int>(_socket->lowest_layer().native_handle());
    ...
    if(!FD_ISSET(nativeSocket,&fileDescriptorSet)){ // timeout
        ...
        sMsg.append(_socket->next_layer().remote_endpoint().address().to_string());
        _ioService.reset();
    ...
    while (!error) {
        std::size_t bytes = read(*_socket, buffer(buff), error);
